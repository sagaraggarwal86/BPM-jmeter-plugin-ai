...
| 2026-03-30 | `gui`, `core` | Defect #1: JSONL load — Test Time Info and overall score blank; fixed via rebuildTableFromRaw() computing firstFiltered/lastFiltered and updateTimeFieldsFromRaw() + updateScoreBoxFromTable(). Defect #2: DONT_START — Chrome still launched, table erased, time info updated; fixed with dontStartPending static volatile flag (set on DONT_START, blocks clones via early-return, cleared only on successful start or testEnded); cachedEngine captured before invokeAndWait; stopTestEngine() dual-path (engine.stopTest + ActionRouter fallback); testActuallyStarted flag prevents testEnded() side-effects on non-started instances; testStarted() GUI guard preserves loaded-file data. Defect #3: new BPM Listener inherits stale table data; configure() now always calls clearDisplayOnly(). Change #1: Start/End Offset always enabled. Change #2: Apply Filters button re-introduced; all auto-trigger listeners removed. Defect (batch 2) #1: Time Info not updated when offset filter changed after file load; rebuildTableFromRaw() now updates time fields in !testRunning path. Defect (batch 2) #2: DONT_START deeper fix for clone instances. Feature #1: continuous End/Duration update every 500 ms via drainGuiQueue() timer; testEnded() wasRunning guard prevents overwrite in DONT_START path. | Done |
| <!-- append rows here --> | | | |
